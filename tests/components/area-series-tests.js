...
test('AreaSeries: basic rendering', t => {
  ...
  t.equal($.find('.area-chart-example').length, 1, 'should find the right number of custom named series');
  ...
  t.equal($.find('.area-chart-example').length, 0, 'should find the right number of custom named series');
  t.end();
});
...
test('AreaSeries: Showcase Example - AreaChartElevated', t => {
  ...
  t.equal($.find('.area-elevated-series-2').length, 1, 'should find the second custom component correctly');
  t.end();
});
