...
test('Highlight: DragableExample', t => {
  ...
  const initialText = '0.00.51.01.52.02.53.03.54.04.55.05.56.06.57.00246810selectionStart: 0,selectionEnd: 0,';
  ...
  for (let i = 0; i < 100; i++) {
    $.find('.rv-mouse-target').simulate('mouseMove', {nativeEvent: {offsetX: 100 + i, offsetY: 100}});
  }
  t.equal($.text(), initialText, 'should find the right text after brushing');
  $.find('.rv-mouse-target').simulate('mouseUp', {nativeEvent: {offsetX: 200, offsetY: 100}});
  t.equal($.text(), '0.00.51.01.52.02.53.03.54.04.55.05.56.06.57.00246810selectionStart: 0.93,selectionEnd: 2.47,', 'should find the right text after brushing');

  // click to clear
  $.find('.rv-mouse-target').simulate('mouseDown', {nativeEvent: {offsetX: 0, offsetY: 0}});
  $.find('.rv-mouse-target').simulate('mouseUp', {nativeEvent: {offsetX: 0, offsetY: 0}});
...
test('Highlight: SelectionPlotExample', t => {
  ...
  t.equal($.find('.selection-example').length, 1, 'should find the custom class is passed through correctly');
  t.equal($.text(), initialText, 'should find the correct text initially');

  $.find('.rv-mouse-target').simulate('mouseDown', {nativeEvent: {offsetX: 100, offsetY: 100}});
  for (let i = 0; i < 100; i++) {
    $.find('.rv-mouse-target').simulate('mouseMove', {nativeEvent: {offsetX: 100, offsetY: 100 + i}});
  }
  t.equal($.text(), '0.51.01.52.02.5681012There are 4 selected points', 'should find the right text after brushing');
  $.find('.rv-mouse-target').simulate('mouseUp', {nativeEvent: {offsetX: 100, offsetY: 200}});
  t.equal($.text(), '0.51.01.52.02.5681012There are 4 selected points', 'should find the right text after brushing');
...
test('Highlight: BidirectionDragChart', t => {
  ...
  t.equal($.text(), '12342468There are 5 selected points', 'should find the right text after brushing');

  $.find('.rv-mouse-target').simulate('mouseUp', {nativeEvent: {offsetX: 200, offsetY: 200}});
  t.equal($.text(), '12342468There are 5 selected points', 'should find the right text after brush finishes');
