...
const MiniChart = (props) => {
  ...
  switch(props.type) {
    case 0:
      // bar charts
      const barData = props.data.map((series, s) =>
        series.map((d, i) => ({
          ...d,
          color: i === props.x ? palette[3] : palette[s]
        }))
      )
      return (<XYPlot {...XYProps} stackBy="y"
          yDomain={[0, props.data.length * 10]}
        >
        {barData.map((d, i) => <VerticalBarSeries
          data={d}
          key={i}
          opacity={
            (props.s !== null && props.s !== i) ? 0.5 : 1
          }
      ...
      case 1:
        // pie charts
        const pieData = props.data.reduce((prev, curr) => [...prev, ...curr], [])
          .reduce((result, d, i) => {
      ...
      case 2:
        // area charts
        return (<XYPlot {...XYProps} stackBy="y"
          yDomain={[0, props.data.length * 20]}
        >
            {props.data.map((d, i) => <AreaSeries
                data={d}
                key={i}
      ...
      case 3:
        // scatterplots
        const scatterData = props.data.map((series, s) =>
          series.map((d, j) => ({
            x: d.xS,
            s,
            y: d.yS,
        ...
        return (<XYPlot {...XYProps} xDomain={[0, 20]}>
          {scatterData.map((d, i) => <MarkSeries
            data={d}
            key={i}
            onSeriesMouseOver={() => props.highlight(i)}
    ...
    default:
      ...
      return (<XYPlot {...XYProps}>
        {props.data.map((d, i) => <LineSeries data={d} key={i}
        ...
        /> )}
        {
          ...
          <LineSeries data={[
            {x: props.x, y: 0},
            {x: props.x, y: 20}
          ]} stroke='#125C77' strokeStyle="dashed" opacity={0.5} /> : null
        }
        {props.x !== null && props.x < props.data[0].length ?
          <MarkSeries
          fillType='literal'
          data={props.data.map((series, s) => ({
            x: props.x,
            y: series[props.x].y,
...
class Hero extends Component {
  ...
  render() {
    ...
    return (
      <div className="Hero" style={{background: '#eeeeef'}}>
        <div className="charts" style={{
        }}>
          {this.state.chartsProps ?
            this.state.chartsProps.map((d, i) => (<div key={i} className="mini-chart"
            style={{
